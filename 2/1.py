...
class TransformApp:
    ...
    def move_ox(self):
        d = simpledialog.askfloat("Перенос OX", "Величина переноса по OX:", minvalue=-10, maxvalue=10, parent=self.root)
        if d is not None:
            self.apply_transform(translation_matrix(d, 0))

    def move_oy(self):
        d = simpledialog.askfloat("Перенос OY", "Величина переноса по OY:", minvalue=-10, maxvalue=10, parent=self.root)
        if d is not None:
